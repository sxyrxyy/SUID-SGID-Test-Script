...
##-- IPF) SUID
echo "SUID - Check easy privesc, exploits and write perms"
echo "https://book.hacktricks.xyz/linux-hardening/privilege-escalation#sudo-and-suid"
...
for s in $suids_files; do
  ...
  elif ! [ "ROOT!" ] && [ -O "$sname" ]; then
    echo "You own the SUID file: $sname" | sed -${E} "s,.*,${SED_RED},"
  elif ! [ "ROOT!" ] && [ -w "$sname" ]; then #If write permision, win found (no check exploits)
